...
export function DecisionPanel({
  ...
  if (request.toolName === "AskUserQuestion" && questions.length > 0) {
    return (
      <div className="rounded-2xl border border-accent/20 bg-accent-subtle p-5">
        <div className="text-xs font-semibold text-accent">{t('decisionPanel.questionFromClaude')}</div>
        {questions.map((q, qIndex) => (
          <div key={qIndex} className="mt-4">
            ...
            {q.multiSelect && <div className="mt-2 text-xs text-muted">{t('decisionPanel.multipleSelections')}</div>}
          </div>
        ))}
  ...
  return (
    <div className="rounded-2xl border border-accent/20 bg-accent-subtle p-5">
      <div className="text-xs font-semibold text-accent">{t('decisionPanel.permissionRequest')}</div>
      <p className="mt-2 text-sm text-ink-700">
        {t('decisionPanel.claudeWantsToUse')} <span className="font-medium">{request.toolName}</span>
      </p>
