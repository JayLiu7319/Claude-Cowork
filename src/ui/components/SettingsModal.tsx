...
export function SettingsModal({ onClose }: SettingsModalProps) {
  ...
  const handleSave = async () => {
    ...
    if (!model.trim()) {
      setError(t('settings.modelRequired'));
      return;
    }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink-900/20 px-4 py-8 backdrop-blur-sm overscroll-contain">
      <div className="w-full max-w-lg rounded-2xl border border-ink-900/5 bg-surface p-6 shadow-elevated">
        <div className="flex items-center justify-between">
          <div className="text-base font-semibold text-ink-800">{t('settings.title')}</div>
          <button
          ...
          <div className="mt-5 grid gap-4">
            <label className="grid gap-1.5">
              ...
              <input
                ...
                name="baseUrl"
                autoComplete="url"
                className="rounded-xl border border-ink-900/10 bg-surface-secondary px-4 py-2.5 text-sm text-ink-800 placeholder:text-muted-light focus:border-accent focus:outline-none focus:ring-1 focus-visible:ring-accent/20 transition-colors"
                placeholder="https://…"
                value={baseURL}
            ...
            {error && (
              <div id="settings-error" className="rounded-xl border border-error/20 bg-error-light px-4 py-2.5 text-sm text-error" role="alert" aria-live="polite">
                {error}
              </div>
            )}

            {success && (
              <div className="rounded-xl border border-success/20 bg-success-light px-4 py-2.5 text-sm text-success" role="status" aria-live="polite">
            ...
            <div className="flex gap-3">
              <button
                className="flex-1 rounded-xl border border-ink-900/10 bg-surface px-4 py-2.5 text-sm font-medium text-ink-700 hover:bg-surface-tertiary transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent"
                onClick={onClose}
                disabled={saving}
              >
                {t('settings.cancel')}
              ...
              <button
                className="flex-1 rounded-xl bg-accent px-4 py-2.5 text-sm font-medium text-white shadow-soft hover:bg-accent-hover transition-colors disabled:cursor-not-allowed disabled:opacity-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent focus-visible:ring-offset-2"
                onClick={handleSave}
